...
float numerical_gradient(std::function<Tensor(Tensor)> f, Tensor x,
                         float eps = 1e-3f) {
  Tensor x_host = x.to({DeviceType::VULKAN, 0});
  float original_val = ((float *)x_host.data())[0];

  ((float *)x_host.data())[0] = original_val + eps;
  Tensor out_pos = f(x_host.to(x.device())).to({DeviceType::VULKAN, 0});

  ((float *)x_host.data())[0] = original_val - eps;
  Tensor out_neg = f(x_host.to(x.device())).to({DeviceType::VULKAN, 0});
...
TEST_P(AutogradTest, AddBackward) {
  ...
  Tensor ga = a.grad().to({DeviceType::VULKAN, 0});
...
TEST(AutogradTest, GradCheckLinear) {
  ...
  Tensor out = model(x);
  out.backward();
  float analytical = ((float *)x.grad().to({DeviceType::VULKAN, 0}).data())[0];
...
TEST_P(AutogradTest, SimpleAutograd) {
  ...
  Tensor grad = w.grad().to({DeviceType::VULKAN, 0});
...
TEST_P(AutogradTest, GradientAccumulatesAcrossMultipleGraphPaths) {
  ...
  Tensor grad = x.grad().to({DeviceType::VULKAN, 0});

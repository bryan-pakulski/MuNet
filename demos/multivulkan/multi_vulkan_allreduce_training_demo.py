#!/usr/bin/env python3
"""Multi-device data-parallel training demo with vulkan-staged gradient all-reduce.

This demo:
1) Creates parameter replicas on multiple Vulkan devices.
2) Runs per-replica forward/backward on batch shards.
3) Aggregates gradients through Vulkan (vulkan staging) to emulate all-reduce.
4) Applies synchronized optimizer steps so replicas stay in sync.

Run:
    python demos/multivulkan/multi_vulkan_allreduce_training_demo.py --steps 20
"""
    ...
    for dev_type in (munet.DeviceType.VULKAN,):
        for idx in range(max_index):
            ...
            try:
                ...
                c = a + b
                c_vulkan = c.to(VULKAN_DEVICE)
                if float(c_vulkan.item()) != 2.0:
                    raise RuntimeError("Vulkan health-check produced incorrect value")
...
def make_model_replicas(devices):
    # Initialize directly on each target device so Vulkan device pairs do not depend on cross-backend parameter copies.
    ws = []
    bs = []
    for dev in devices:
...
def max_tensor_drift(tensors):
    if len(tensors) < 2:
        return 0.0
    base = np.array(tensors[0].detach().to(VULKAN_DEVICE), copy=False)
    max_drift = 0.0
    for t in tensors[1:]:
        other = np.array(t.detach().to(VULKAN_DEVICE), copy=False)
...
def main():
    ...
    ap.add_argument("--num-devices", type=int, default=2,
                    help="Number of Vulkan replicas to train with.")
    ...
    if len(devices) < max(2, args.num_devices):
        print(f"Need at least {max(2, args.num_devices)} healthy Vulkan devices.")
        return
    ...
    os.environ["MUNET_ALLREDUCE_GROUP"] = "python_demo_multivulkan"
    ...
    for step in range(args.steps):
        ...
        for rank, dev in enumerate(devices):
            ...
            loss.backward()
            losses.append(float(loss.detach().to(VULKAN_DEVICE).item()))
        ...
        w0 = np.array(ws[0].detach().to(VULKAN_DEVICE), copy=False)
        w1 = np.array(ws[1].detach().to(VULKAN_DEVICE), copy=False)
    ...
    learned_w = np.array(ws[0].detach().to(VULKAN_DEVICE), copy=False)
    learned_b = np.array(bs[0].detach().to(VULKAN_DEVICE), copy=False)
    print(

"""Offline Vulkan/default and CPU fallback checks of the documented train/resume/inference entry points."""
import importlib
import json
import sys
...
def invoke(module, arguments, monkeypatch, device):
    fallback = ["--device", "cpu"] if device == "cpu" else []
    monkeypatch.setattr(sys, "argv", [module, *map(str, arguments), *fallback])
...
def test_train_resume_and_infer(name, arguments, tmp_path, monkeypatch, capsys, device):
    ...
    invoke(f"examples.{name}.train", arguments, monkeypatch, device)
    checkpoint = tmp_path / "training.mnet"
    invoke(f"examples.{name}.train", [*arguments, "--resume", checkpoint], monkeypatch, device)
    assert load_state(checkpoint)["steps"] == 2
    metrics = json.loads((tmp_path / "metrics.json").read_text())
    assert metrics["steps"] == 2 and (tmp_path / "inference.mnet").is_file()
    assert (metrics["runtime"]["submissions"] > 0) == (device == "vulkan")
    assert all(np.isfinite(value) for value in metrics["final"].values())
    capsys.readouterr()
    if name == "language_model":
        invoke("examples.language_model.generate", ["--checkpoint", checkpoint, "--prompt", "the", "--tokens", "4", "--output", tmp_path / "generated.txt"], monkeypatch, device)
        assert len((tmp_path / "generated.txt").read_text()) == 7
    elif name == "mnist":
        invoke("examples.mnist.infer", ["--checkpoint", checkpoint, "--image", tmp_path / "sample.png"], monkeypatch, device)
        ...
        invoke("examples.segmentation.infer", ["--checkpoint", checkpoint, "--image", tmp_path / "sample.png", "--output", output], monkeypatch, device)

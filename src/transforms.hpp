...
namespace munet {                                                                                                                                                                        
	 ...
	 inline Tensor Flatten::backward(const Tensor& grad_output) {                                                                                                                         
					 ...
					 if (grad_output.device_ == Device::CUDA) {                                                                                                                                   
									 cudaMemcpy(grad_input.data(), grad_output.data(), grad_output.bytes(), cudaMemcpyDeviceToDevice);                                                                    
									 return grad_input;                                                                                                                                                   
					 }                                                                                                                                                                            
#endif                                                                                                                                                                                   
	 ...
	 private:                                                                                                                                                                             
					 ...
					 int stride_;                                                                                                                                                                 
					 std::vector<int> input_shape_;                                                                                                                                               
					 Tensor max_indices_{std::vector<int>{}}; // Changed from std::vector to Tensor for GPU support                                                                               
	 };                                                                                                                                                                                   
	 ...
	 inline Tensor MaxPool2D::forward(const Tensor& input) {                                                                                                                              
					 ...
					 // Allocate indices on same device. Treat as FP32 size-wise (4 bytes per index)                                                                                              
...
#ifdef MUNET_USE_CUDA                                                                                                                                                                    
					 if (input.device_ == Device::CUDA) {                                                                                                                                         
									 cuda_kernels::maxpool_forward(in_ptr, out_ptr, idx_ptr, N, C, H, W, OH, OW, kernel_size_, stride_);                                                                  
									 return output;                                                                                                                                                       
					 }                                                                                                                                                                            
#endif                                                                                                                                                                                   
...
#ifdef MUNET_USE_CUDA                                                                                                                                                                    
					 if (input.device_ == Device::CUDA) {                                                                                                                                         
									 cuda_kernels::conv2d_forward(in_ptr, w_ptr, b_ptr, out_ptr, N, C, out_channels_, H, W, OH, OW, kernel_size_, stride_, padding_);                                     
									 return output;                                                                                                                                                       
					 }                                                                                                                                                                            
#endif                                                                                                                                                                                   
     ...
     if (grad_output.device_ == Device::CUDA) {                                                                                                                                           
         cuda_kernels::conv2d_backward(go_ptr, in_ptr, w_ptr, gi_ptr, gw_ptr, gb_ptr,                                                                                                     
             N, C, out_channels_, H, W, OH, OW, kernel_size_, stride_, padding_);                                                                                                         
         return grad_input;                                                                                                                                                               
     }                                                                                                                                                                                    
 #endif 
    ...
    class BatchNorm2D : public Layer {                                                                                                                                                   
     public:                                                                                                                                                                              
         inline BatchNorm2D(int num_features, float eps = 1e-5f, float momentum = 0.1f);                                                                                                  
         inline Tensor forward(const Tensor& input) override;                                                                                                                             
         inline Tensor backward(const Tensor& grad_output) override;                                                                                                                      
                                                                                                                                                                                          
         inline std::unordered_map<std::string, Tensor*> get_parameters() override {                                                                                                      
             return {{"weight", &weight_}, {"bias", &bias_},                                                                                                                              
                     {"running_mean", &running_mean_}, {"running_var", &running_var_}};                                                                                                   
         }                                                                                                                                                                                
         inline std::unordered_map<std::string, Tensor*> get_gradients() override {                                                                                                       
             return {{"weight", weight_.grad()}, {"bias", bias_.grad()}};                                                                                                                 
         }                                                                                                                                                                                
         inline std::string get_onnx_op_type() const override { return "BatchNormalization"; }                                                                                            
     ...
     inline Tensor BatchNorm2D::forward(const Tensor& input) {                                                                                                                            
 ...
 #ifdef MUNET_USE_CUDA                                                                                                                                                                    
         if (input.device_ == Device::CUDA) {                                                                                                                                             
             float* sm_ptr = training_ ? static_cast<float*>(save_mean_.data()) : nullptr;                                                                                                
             float* sv_ptr = training_ ? static_cast<float*>(save_var_.data()) : nullptr;                                                                                                 
             float* sis_ptr = training_ ? static_cast<float*>(save_inv_std_.data()) : nullptr;                                                                                            
                                                                                                                                                                                          
             cuda_kernels::batchnorm2d_forward(in_ptr, out_ptr, sm_ptr, sv_ptr, sis_ptr,                                                                                                  
                 rm_ptr, rv_ptr, w_ptr, b_ptr, N, C, H, W, eps_, momentum_, training_);                                                                                                   
             return output;                                                                                                                                                               
         }                                                                                                                                                                                
 #endif                                                                                                                                                                                   
         ...
         if (training_) {                                                                                                                                                                 
             ...
             for (int c = 0; c < C; ++c) {                                                                                                                                                
                 ...
                 for (int n = 0; n < N; ++n) {                                                                                                                                            
                     for (int h = 0; h < H; ++h) {                                                                                                                                        
                         for (int w = 0; w < W; ++w) sum += in_ptr[((n * C + c) * H + h) * W + w];                                                                                        
                     }                                                                                                                                                                    
                 }                                                                                                                                                                        
                 float mean = sum / M;                                                                                                                                                    
                 ...
                 rv_ptr[c] = (1.0f - momentum_) * rv_ptr[c] + momentum_ * (var * M / (M > 1 ? M - 1 : 1));                                                                                
             ...
             for (int c = 0; c < C; ++c) {                                                                                                                                                
                 ...
                 for (int n = 0; n < N; ++n) {                                                                                                                                            
                     for (int h = 0; h < H; ++h) {                                                                                                                                        
                         for (int w = 0; w < W; ++w) {                                                                                                                                    
                             int idx = ((n * C + c) * H + h) * W + w;                                                                                                                     
                             out_ptr[idx] = (in_ptr[idx] - rm_ptr[c]) * inv_std * w_ptr[c] + b_ptr[c];                                                                                    
                         }                                                                                                                                                                
                     }                                                                                                                                                                    
                 }                                                                                                                                                                        
             }                                                                                                                                                                            
     ...
     inline Tensor BatchNorm2D::backward(const Tensor& grad_output) {                                                                                                                     
         if (!training_) throw std::runtime_error("Cannot backward BatchNorm in eval mode");                                                                                              
             ...
             for (int n = 0; n < N; ++n) {                                                                                                                                                
                 for (int h = 0; h < H; ++h) {                                                                                                                                            
                     for (int w = 0; w < W; ++w) {                                                                                                                                        
                         ...
                         float go_val = go_ptr[idx];                                                                                                                                      
                         float xhat = (in_ptr[idx] - mean) * inv_std;                                                                                                                     
                         gi_ptr[idx] = (w_c * inv_std / M) * (M * go_val - sum_go - xhat * sum_go_xhat);                                                                                  
                     }                                                                                                                                                                    
                 }                                                                                                                                                                        
             }                                                                                                                                                                            
         }                                                                                                                                                                                
...
#ifdef MUNET_USE_CUDA                                                                                                                                                                    
					 if (input.device_ == Device::CUDA) {                                                                                                                                         
							 cuda_kernels::upsample_forward(in_ptr, out_ptr, N, C, H, W, OH, OW, scale_factor_);                                                                                      
							 return output;                                                                                                                                                           
					 }                                                                                                                                                                            
#endif                                                                                                                                                                                   
									 ...
									 for (int c = 0; c < C; ++c) {                                                                                                                                        
													 for (int oh = 0; oh < OH; ++oh) {                                                                                                                            
																	 for (int ow = 0; ow < OW; ++ow) {                                                                                                                    
																					 ...
																					 out_ptr[n*(C*OH*OW) + c*(OH*OW) + oh*OW + ow] = in_ptr[n*(C*H*W) + c*(H*W) + ih*W + iw];                                                     
																	 }                                                                                                                                                    
													 }                                                                                                                                                            
									 }                                                                                                                                                                    
					 }                                                                                                                                                                            
 ...
 #ifdef MUNET_USE_CUDA                                                                                                                                                                    
     if (grad_output.device_ == Device::CUDA) {                                                                                                                                           
         cuda_kernels::upsample_backward(go_ptr, gi_ptr, N, C, H, W, OH, OW, scale_factor_);                                                                                              
         return grad_input;                                                                                                                                                               
     }                                                                                                                                                                                    
 #endif      
									 ...
									 for (int c = 0; c < C; ++c) {                                                                                                                                        
													 for (int oh = 0; oh < OH; ++oh) {                                                                                                                            
																	 for (int ow = 0; ow < OW; ++ow) {                                                                                                                    
																					 int ih = oh / scale_factor_;                                                                                                                 
																					 int iw = ow / scale_factor_;                                                                                                                 
																					 gi_ptr[n*(C*H*W) + c*(H*W) + ih*W + iw] += go_ptr[n*(C*OH*OW) + c*(OH*OW) + oh*OW + ow];                                                     
																	 }                                                                                                                                                    
													 }                                                                                                                                                            
									 }                                                                                                                                                                    
					 }                                                                                                                                                                            

...
namespace cuda_kernels {
		void softmax_forward(const float* in, float* out, int batch_size, int num_classes);                                                                                                      
		void softmax_backward(const float* go, const float* out, float* gi, int batch_size, int num_classes);                                                                                    
		void maxpool_forward(const float* in, float* out, int* indices, int N, int C, int H, int W, int OH, int OW, int K, int S);                                                               
    ...
    void conv2d_backward(const float* go, const float* in, const float* w, float* gi, float* gw, float* gb,                                                                              
		...
		void batchnorm2d_forward(const float* in, float* out, float* save_mean, float* save_var, float* save_inv_std, float* run_mean, float* run_var, const float* weight, const float*     
		 bias, int N, int C, int H, int W, float eps, float momentum, bool training);                                                                                                             
																																																																																															
				 void batchnorm2d_backward(const float* go, const float* in, float* gi, float* gw, float* gb, const float* mean, const float* inv_std, const float* weight, int N, int C, int H, int  
		 W); 
    ...
    void upsample_backward(const float* go, float* gi, int N, int C, int H, int W, int OH, int OW, int SF);
		float mse_loss_cuda(const float* p, const float* t, float* grad_output, int size);

...
namespace munet {
	class Tensor {
	public:
			Tensor(const std::vector<int>& shape, Device dev = Device::CPU, DataType dtype = DataType::FP32);
			~Tensor();
	...
	private:
			...
			void* data_ptr_ = nullptr; // Void pointer handles mixed precision (float vs half)
	...
	inline void Tensor::to_gpu() {
			...
			void* new_ptr;
			if (cudaMalloc(&new_ptr, bytes()) != cudaSuccess) throw std::runtime_error("CUDA alloc failed");
			if (cudaMemcpy(new_ptr, data_ptr_, bytes(), cudaMemcpyHostToDevice) != cudaSuccess) {
					cudaFree(new_ptr);
	...
	inline void Tensor::to_cpu() {
			...
			if (cudaMemcpy(new_ptr, data_ptr_, bytes(), cudaMemcpyDeviceToHost) != cudaSuccess) {
					std::free(new_ptr);
					throw std::runtime_error("CUDA memcpy failed");
			}
			deallocate();
  ...
  inline void Tensor::copy_from(const void* host_data, size_t size) {
      if (size != bytes()) throw std::runtime_error("Size mismatch in copy_from");
      ...
      if (device_ == Device::CPU) {
          ...
          if (cudaMemcpy(data_ptr_, host_data, size, cudaMemcpyHostToDevice) != cudaSuccess) {
              throw std::runtime_error("CUDA memcpy failed");
          }
      }
#endif

...
public:
  ...
  virtual std::unordered_map<std::string, Tensor *> get_parameters() {
    return {};
  }
  virtual std::unordered_map<std::string, Tensor *> get_gradients() {
    return {};
  }
...
inline Tensor Linear::forward(const Tensor &input) {
  ...
  Tensor output({batch_size, out_features_}, input_cache_.device_,
                input_cache_.dtype_);
  ...
  if (input.device_ == Device::CUDA) {
    ...
    cublasSgemm(Context::instance().cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N,
                out_features_, batch_size, in_features_, &alpha, w_ptr,
                out_features_, in_ptr, in_features_, &beta, out_ptr,
                out_features_);
  ...
  for (int b = 0; b < batch_size; ++b) {
    for (int out_c = 0; out_c < out_features_; ++out_c) {
      ...
      for (int in_c = 0; in_c < in_features_; ++in_c) {
        sum += in_ptr[b * in_features_ + in_c] *
               w_ptr[in_c * out_features_ + out_c];
      }
...
inline Tensor Linear::backward(const Tensor &grad_output) {
  ...
  Tensor grad_input({batch_size, in_features_}, input_cache_.device_,
                    input_cache_.dtype_);
  grad_input.zero();
  ...
  if (grad_output.device_ == Device::CUDA) {
    ...
    cublasSgemm(Context::instance().cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N,
                in_features_, batch_size, out_features_, &alpha, w_ptr,
                out_features_, go_ptr, out_features_, &beta, gi_ptr,
                in_features_);

    // 2. Grad Weight = Input^T * Grad Output
    cublasSgemm(Context::instance().cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T,
                out_features_, in_features_, batch_size, &alpha, go_ptr,
                out_features_, in_ptr, in_features_, &beta, gw_ptr,
                out_features_);
  ...
  for (int b = 0; b < batch_size; ++b) {
    for (int out_c = 0; out_c < out_features_; ++out_c) {
      ...
      for (int in_c = 0; in_c < in_features_; ++in_c) {
        gw_ptr[in_c * out_features_ + out_c] +=
            in_ptr[b * in_features_ + in_c] * go_val;
        gi_ptr[b * in_features_ + in_c] +=
            go_val * w_ptr[in_c * out_features_ + out_c];
      }
    }
  }
  return grad_input;

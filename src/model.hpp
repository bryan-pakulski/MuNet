...
namespace munet {
	...
	public:
			...
			void train() { for (auto& l : layers_) l->train(); }
			void eval() { for (auto& l : layers_) l->eval(); }
	...
	inline Tensor Model::forward(const Tensor& input) {
			if (layers_.empty()) return input.clone();
...
#else
			throw std::runtime_error("MuNet compiled without ONNX support. Define MUNET_USE_ONNX.");
#endif
	}
	...
	inline void Model::save_weights(const std::string& path) const {
			...
			for (const auto& layer : layers_) {
					...
					for (const auto& [name, tensor] : params) {
							...
							out.write(reinterpret_cast<const char*>(shape.data()), ndims * sizeof(int));
	...
	inline void Model::load_weights(const std::string& path) {
			...
			if (magic != 0x4D554E54) throw std::runtime_error("Invalid weight file format");
			
			uint32_t num_layers;
			in.read(reinterpret_cast<char*>(&num_layers), sizeof(num_layers));
			if (num_layers != layers_.size()) throw std::runtime_error("Model layer count mismatch");
			
			for (size_t i = 0; i < num_layers; ++i) {
					auto params = layers_[i]->get_parameters();
					uint32_t num_params;
					in.read(reinterpret_cast<char*>(&num_params), sizeof(num_params));
					if (num_params != params.size()) throw std::runtime_error("Layer parameter count mismatch");
					...
					for (uint32_t p = 0; p < num_params; ++p) {
							...
							if (params.find(name) == params.end()) throw std::runtime_error("Param " + name + " not found");
							...
							if (shape != params[name]->shape()) throw std::runtime_error("Shape mismatch for " + name);
							
							uint64_t bytes;
							in.read(reinterpret_cast<char*>(&bytes), sizeof(bytes));
							if (bytes != params[name]->bytes()) throw std::runtime_error("Byte size mismatch for " + name);

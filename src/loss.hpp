...
inline float cross_entropy_loss(const Tensor &logits, const Tensor &targets,
                                Tensor &grad_output) {
  if (logits.size() != targets.size())
    throw std::runtime_error("CE size mismatch");
  if (logits.device_ != targets.device_)
    throw std::runtime_error(
        "Device mismatch: logits and targets must be on the same device");
...
inline float mse_loss(const Tensor &pred, const Tensor &target,
                      Tensor &grad_output) {

...
PYBIND11_MODULE(munet, m) {
  ...
  py::enum_<amp::GradScaler::Mode>(amp_mod, "GradScalerMode")
      .value("Dynamic", amp::GradScaler::Mode::Dynamic)
      .value("Static", amp::GradScaler::Mode::Static);

  py::class_<amp::GradScaler>(amp_mod, "GradScaler")
      .def(py::init<float, float, float, int, amp::GradScaler::Mode>(),
           py::arg("init_scale") = 65536.0f,
           py::arg("growth_factor") = 2.0f,
           py::arg("backoff_factor") = 0.5f,
           py::arg("growth_interval") = 2000,
           py::arg("mode") = amp::GradScaler::Mode::Dynamic)
...
def _autocast(dtype=None, _munet=munet):
    if dtype is None:
        dtype = _munet.DataType.Float16
    class _AutoCtx:
        def __enter__(self):
            self._g = _munet.amp.AutoCastGuard(dtype)
            return self
...
def _autocast_policy(op, enabled, _munet=munet):
    class _PolicyCtx:
        def __enter__(self):
            self._g = _munet.amp.AutocastPolicyGuard(op, enabled)
            return self
        def __exit__(self, exc_type, exc, tb):
            self._g = None
            return False

				...
				void main() {
						...
						if (p.tA == 0 && p.tB == 0) {
								...
								for (int t = 0; t < tiles; ++t) {
										...
										As[ly][lx] = (active && kA < p.K) ? a[row * p.K + kA] : 0.0;
										Bs[ly][lx] = (active && kB < p.K) ? b[kB * p.N + col] : 0.0;
						...
						} else if (active && p.tA == 1 && p.tB == 0) {
								for (int k = 0; k < p.K; ++k)
										sum += a[k * p.M + row] * b[k * p.N + col];
						} else if (active && p.tA == 0 && p.tB == 1) {
								for (int k = 0; k < p.K; ++k)

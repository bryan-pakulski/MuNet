...
std::string extract_npy_header_string(const std::string &header,
  ...
  if (value_end == std::string::npos) {
    throw std::runtime_error("NPY header key '" + key + "' has unterminated string");
  }
  return header.substr(value_pos, value_end - value_pos);
}

bool extract_npy_header_bool(const std::string &header, const std::string &key) {
  const std::string needle = "'" + key + "':";
  const size_t key_pos = header.find(needle);
  if (key_pos == std::string::npos) {
    throw std::runtime_error("NPY header missing key '" + key + "'");
...
std::vector<size_t> extract_npy_header_shape(const std::string &header) {
  ...
  const std::string inside = trim(header.substr(value_pos + 1, value_end - value_pos - 1));
  if (inside.empty()) {
    return {};
  }
...
NpyArray parse_npy_array(const std::vector<uint8_t> &bytes) {
  ...
  if (bytes.size() < 10 || !std::equal(std::begin(kMagic), std::end(kMagic), bytes.begin())) {
...
std::unordered_map<std::string, std::vector<uint8_t>> parse_npz_archive(
    const std::string &path) {
  const std::vector<uint8_t> bytes = read_file_bytes(path);
  if (bytes.size() < 22) {
    throw std::runtime_error("Serialized artifact is too small to be a valid .npz archive: " + path);
  }

  size_t eocd_offset = std::string::npos;
  const size_t min_search = bytes.size() > (22 + 65535) ? bytes.size() - (22 + 65535) : 0;
  ...
  if (eocd_offset == std::string::npos) {
    throw std::runtime_error("Unable to locate .npz central directory in artifact: " + path);
  }
  ...
  if (static_cast<size_t>(central_dir_offset) + central_dir_size > bytes.size()) {
    throw std::runtime_error("Invalid .npz central directory bounds in artifact: " + path);
  }
  ...
  for (uint16_t i = 0; i < entry_count; ++i) {
    if (read_u32_le(bytes, pos) != 0x02014b50U) {
      throw std::runtime_error("Invalid .npz central directory entry in artifact: " + path);
    }
    ...
    const size_t header_end = file_name_offset + file_name_len + extra_len + comment_len;
    if (header_end > bytes.size()) {
      throw std::runtime_error("Invalid .npz central directory filename bounds in artifact: " + path);
    }

    const std::string filename(
        reinterpret_cast<const char *>(bytes.data() + static_cast<std::ptrdiff_t>(file_name_offset)),
        file_name_len);

    if (compression_method != 0) {
      throw std::runtime_error(
          "Compressed .npz entries are not supported by the C++ deploy loader: " + filename);
    }
    if (compressed_size != uncompressed_size) {
      throw std::runtime_error("Unexpected compressed size mismatch in serialized artifact: " + filename);
    }

    if (static_cast<size_t>(local_header_offset) + 30 > bytes.size() ||
        read_u32_le(bytes, local_header_offset) != 0x04034b50U) {
      throw std::runtime_error("Invalid .npz local header in artifact: " + filename);
    }

    const uint16_t local_name_len = read_u16_le(bytes, local_header_offset + 26);
    const uint16_t local_extra_len = read_u16_le(bytes, local_header_offset + 28);
    const size_t data_offset = static_cast<size_t>(local_header_offset) + 30 + local_name_len + local_extra_len;
    const size_t data_end = data_offset + uncompressed_size;
    if (data_end > bytes.size()) {
      throw std::runtime_error("Invalid .npz entry bounds in artifact: " + filename);
    }

    entries.emplace(filename,
                    std::vector<uint8_t>(bytes.begin() + static_cast<std::ptrdiff_t>(data_offset),
...
std::string decode_unicode_scalar(const NpyArray &array) {
  ...
  if (array.descr.size() < 3 || array.descr[0] != '<' || array.descr[1] != 'U') {
    throw std::runtime_error("Expected little-endian unicode scalar array");
  }

  const size_t char_count = static_cast<size_t>(std::stoull(array.descr.substr(2)));
  ...
  for (size_t i = 0; i < char_count; ++i) {
    const uint32_t codepoint = static_cast<uint32_t>(array.data[i * 4]) |
    ...
    if (codepoint > 0x7f) {
      throw std::runtime_error("Only ASCII unicode scalar metadata is supported");
    }
...
std::string decode_bytes_scalar(const NpyArray &array) {
  if (array.descr.size() < 3 || (array.descr[0] != '|' && array.descr[0] != '<') ||
      array.descr[1] != 'S') {
    throw std::runtime_error("Expected bytes scalar array");
  }
  const size_t byte_count = static_cast<size_t>(std::stoull(array.descr.substr(2)));
  if (array.data.size() < byte_count) {
    throw std::runtime_error("Bytes scalar payload is truncated");
  }
  std::string value(reinterpret_cast<const char *>(array.data.data()), byte_count);
...
std::string decode_string_scalar(const NpyArray &array) {
  ...
  throw std::runtime_error("Unsupported string scalar descriptor: " + array.descr);
}

int64_t decode_int_scalar(const NpyArray &array) {
  if (!(array.shape.empty() || (array.shape.size() == 1 && array.shape[0] == 1))) {
  ...
  if (array.descr == "<i8") {
    ...
    int64_t value = 0;
    for (int i = 0; i < 8; ++i) {
      value |= static_cast<int64_t>(array.data[static_cast<size_t>(i)]) << (8 * i);
    }
    return value;
  }
  if (array.descr == "<i4") {
    ...
    int32_t value = 0;
    for (int i = 0; i < 4; ++i) {
      value |= static_cast<int32_t>(array.data[static_cast<size_t>(i)]) << (8 * i);
    }
    return value;
  }
  throw std::runtime_error("Unsupported integer scalar descriptor: " + array.descr);
}
...
bool decode_bool_scalar(const NpyArray &array) {
  ...
  throw std::runtime_error("Unsupported boolean scalar descriptor: " + array.descr);
}
...
DataType data_type_from_npy_descr(const std::string &descr) {
  ...
  throw std::runtime_error("Unsupported tensor dtype in serialized artifact: " + descr);
}
...
Tensor tensor_from_npy_array(const NpyArray &array) {
  ...
  for (size_t dim : array.shape) {
    if (dim > static_cast<size_t>(std::numeric_limits<int>::max())) {
      throw std::runtime_error("Serialized tensor dimension exceeds MuNet Shape range");
    }
...
private:
  JsonValue parse_value() {
    ...
    throw std::runtime_error(std::string("Unsupported JSON token starting with '") + ch + "'");
  }
...
const JsonValue &required_field(const JsonValue::Object &object,
  ...
  if (it == object.end()) {
    throw std::runtime_error("Serialized module config is missing required field '" + key + "'");
  }
  return it->second;
}
...
std::shared_ptr<nn::Module> build_module_from_config(const JsonValue &config_value) {
  if (!config_value.is_object()) {
    throw std::runtime_error("Serialized module config root must be a JSON object");
  }
  ...
  if (type == "Linear") {
    return std::make_shared<nn::Linear>(required_field(config, "in_features").as_int(),
                                        required_field(config, "out_features").as_int(),
                                        required_field(config, "bias").as_bool(), options);
  }
  if (type == "Conv2d") {
    return std::make_shared<nn::Conv2d>(required_field(config, "in_channels").as_int(),
  ...
  if (type == "MaxPool2d") {
    return std::make_shared<nn::MaxPool2d>(required_field(config, "kernel_size").as_int(),
                                           required_field(config, "stride").as_int(),
                                           required_field(config, "padding").as_int());
  }
  if (type == "BatchNorm2d") {
    return std::make_shared<nn::BatchNorm2d>(required_field(config, "num_features").as_int(),
                                             static_cast<float>(required_field(config, "eps").as_number()),
                                             static_cast<float>(required_field(config, "momentum").as_number()),
                                             options);
  }
  if (type == "Upsample") {
    return std::make_shared<nn::Upsample>(required_field(config, "scale_factor").as_int());
  }
  ...
  if (type == "LeakyReLU") {
    return std::make_shared<nn::LeakyReLU>(
        static_cast<float>(required_field(config, "negative_slope").as_number()));
  }
  if (type == "Dropout") {
  ...
  if (type == "Embedding") {
    return std::make_shared<nn::Embedding>(required_field(config, "num_embeddings").as_int(),
                                           required_field(config, "embedding_dim").as_int(),
                                           options);
  }
  if (type == "LayerNorm") {
    return std::make_shared<nn::LayerNorm>(required_field(config, "normalized_shape").as_int(),
                                           static_cast<float>(required_field(config, "eps").as_number()),
                                           options);
  }
  if (type == "MultiHeadAttention") {
    return std::make_shared<nn::MultiHeadAttention>(required_field(config, "embed_dim").as_int(),
                                                    required_field(config, "num_heads").as_int(),
                                                    required_field(config, "causal").as_bool(),
                                                    options);
  }
  if (type == "Flatten") {
...
void copy_tensor_into(Tensor &target, const Tensor &source) {
  if (target.shape() != source.shape()) {
    throw std::runtime_error("Serialized tensor shape mismatch for '" + target.name() +
                             "': expected " + to_string(target.shape()) +
                             ", got " + to_string(source.shape()));
  }
  ...
  if (prepared.bytes() != target.bytes()) {
    throw std::runtime_error("Serialized tensor byte-size mismatch for '" + target.name() + "'");
  }
...
std::vector<std::string> parse_manifest_tensor_names(const std::string &json_text) {
...
ParsedArtifact parse_and_validate_artifact(const std::string &path) {
  ...
  for (const auto &entry : entries) {
    ...
    if (name.rfind("__", 0) == 0) {
      if (name == "__format_name__") {
        ...
        artifact.tensor_manifest = parse_manifest_tensor_names(decode_string_scalar(array));
      }
      continue;
    }
    ...
    for (const auto &token : kForbiddenTrainingKeyTokens) {
      if (lowered.find(token) != std::string::npos) {
        throw std::runtime_error(
            "Unsupported training/checkpoint payload key in deploy artifact: " + name);
      }
    }
  ...
  if (!artifact.tensor_manifest.empty() && artifact.tensor_manifest != payload_names) {
    throw std::runtime_error("Serialized tensor manifest does not match payload keys");
  }

  if (artifact.format_name != kSerializationFormatName) {
    throw std::runtime_error("Unsupported serialization format name: " + artifact.format_name);
  }
  ...
  if (!artifact.legacy_tag.empty() && artifact.legacy_tag != kSerializationLegacyTag) {
    throw std::runtime_error("Unsupported legacy serialization tag: " + artifact.legacy_tag);
  }
  if (!artifact.producer.empty() && artifact.producer != kSerializationProducer) {
    throw std::runtime_error("Unsupported serialization producer: " + artifact.producer);
  }
  if (artifact.artifact_kind != kSerializationArtifactKind) {
    throw std::runtime_error("Unsupported serialization artifact kind: " + artifact.artifact_kind);
  }
  if (artifact.artifact_scope != kSerializationArtifactScope) {
    throw std::runtime_error("Unsupported serialization artifact scope: " + artifact.artifact_scope);
  }
  if (artifact.default_load_mode != kSerializationDefaultLoadMode) {
    throw std::runtime_error("Unsupported deploy load mode: " + artifact.default_load_mode);
  }
  ...
  if (artifact.recommended_loader != kSerializationRecommendedLoader) {
    throw std::runtime_error("Unsupported recommended loader: " + artifact.recommended_loader);
  }
  ...
  if (artifact.config_json.empty()) {
    throw std::runtime_error(
        "Serialized deploy artifact does not contain architecture config required for C++ reconstruction");
  }

  return artifact;
}

void apply_state(const std::shared_ptr<core::Module> &module,
                 const std::unordered_map<std::string, Tensor> &serialized_tensors) {
  if (!module) {
    throw std::runtime_error("Cannot load serialized weights into a null module");
  }
  ...
  for (const auto &entry : serialized_tensors) {
    ...
    if (it == named.end()) {
      throw std::runtime_error("Serialized tensor '" + entry.first +
                               "' does not match any named parameter/buffer in the reconstructed module");
    }

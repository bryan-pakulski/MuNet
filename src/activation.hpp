...
namespace munet {
	...
	inline Tensor ReLU::backward(const Tensor& grad_output) {
			Tensor grad_input(grad_output.shape(), grad_output.device_, grad_output.dtype_);
	...
	inline Tensor Softmax::forward(const Tensor& input) {
			...
			int num_classes = input.shape().size() > 1 ? input.shape()[1] : input.shape()[0];
	...
	inline Tensor Softmax::backward(const Tensor& grad_output) {
			...
			int num_classes = grad_output.shape().size() > 1 ? grad_output.shape()[1] : grad_output.shape()[0];

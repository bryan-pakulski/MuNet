...
- [~] Add autocast context for forward op dispatch (C++ + Python) (API + bindings + Tensor forward-op autocast integration for arithmetic/activation/loss, plus explicit `AutocastOp` policy table used by Tensor entrypoints; spatial/normalization ops currently mapped to skip until safe mixed-precision kernels land; broader module coverage still pending).

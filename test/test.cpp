...
void test_linear_backward() {
    ...
    go_ptr[0] = 1.0f; go_ptr[1] = 1.0f; go_ptr[2] = 1.0f;
...
void test_relu() {
    ...
    float* in_ptr = static_cast<float*>(input.data());
    in_ptr[0] = -1.0f; in_ptr[1] = 0.0f; in_ptr[2] = 2.0f;
    ...
    Tensor grad_out({1, 3}, Device::CPU, DataType::FP32);
    float* go_ptr = static_cast<float*>(grad_out.data());
    go_ptr[0] = 5.0f; go_ptr[1] = 5.0f; go_ptr[2] = 5.0f;
...
void test_classification_training() {
    ...
    // Sample 1
    in_ptr[0] = 0.5f; in_ptr[1] = 0.1f;
    // Sample 2
    in_ptr[2] = -0.5f; in_ptr[3] = -0.1f;
    ...
    t_ptr[0] = 1.0f; t_ptr[1] = 0.0f; t_ptr[2] = 0.0f;
    // Sample 2 belongs to class 2
    t_ptr[3] = 0.0f; t_ptr[4] = 0.0f; t_ptr[5] = 1.0f;
...
void test_save_load() {
    ...
    Tensor input({1, 3}, Device::CPU, DataType::FP32);
    float* in_ptr = static_cast<float*>(input.data());
    in_ptr[0] = 0.5f; in_ptr[1] = -0.2f; in_ptr[2] = 0.1f;
...
void test_flatten() {
    ...
    assert(in_shape[0] == 2 && in_shape[1] == 3 && in_shape[2] == 4 && in_shape[3] == 4);
...
void test_maxpool2d() {
    ...
    //  8  9 10 11
    // 12 13 14 15
    for (int i = 0; i < 16; ++i) in_ptr[i] = static_cast<float>(i);
    ...
    float* go_ptr = static_cast<float*>(grad_out.data());
    for (int i = 0; i < 4; ++i) go_ptr[i] = 1.0f;
...
void test_conv2d() {
    ...
    float* in_ptr = static_cast<float*>(input.data());
    for (int i = 0; i < 9; ++i) in_ptr[i] = 1.0f;
...
void test_cnn_integration() {
    ...
    float* in_ptr = static_cast<float*>(input.data());
    for(int i=0; i<36; ++i) in_ptr[i] = static_cast<float>(i % 4);
    ...
    for(auto* p : params) initial_weights.push_back(static_cast<float*>(p->data())[0]);
    ...
    for(size_t i = 0; i < params.size(); ++i) {
        if (std::abs(static_cast<float*>(params[i]->data())[0] - initial_weights[i]) > 1e-6f) {
...
void test_error_handling() {
    ...
    try {
        ...
        l.forward(bad_input); // Should compute garbage or crash if not careful, but size check is implicit in loops
...
// Helper to copy parameters from one layer to another to ensure identical start state
...
void test_linear_gpu_vs_cpu() {
    ...
    for(int i=0; i<4*32; ++i) in_ptr[i] = (float)i / 100.0f;
    ...
    for(size_t i=0; i<cpu_out.size(); ++i) {
        if (!is_close(cpu_d[i], gpu_d[i], 1e-4f)) {
            std::cerr << "Mismatch at index " << i << " CPU:" << cpu_d[i] << " GPU:" << gpu_d[i] << std::endl;
    ...
    for(int i=0; i<4*16; ++i) g_ptr[i] = 1.0f;
    ...
    for(size_t i=0; i<cpu_grad_in.size(); ++i) {
        if (!is_close(cgi[i], ggi[i], 1e-4f)) throw std::runtime_error("Linear Backward Input Grad mismatch");
    }
    ...
    for(size_t i=0; i<cpu_w->size(); ++i) {
        if (!is_close(cgw[i], ggw[i], 1e-4f)) throw std::runtime_error("Linear Backward Weight Grad mismatch");
    }
...
void test_relu_gpu() {
    ...
    for(int i=0; i<10; ++i) p[i] = (i % 2 == 0) ? (float)i : -(float)i;
    ...
    for(int i=0; i<10; ++i) {
        ...
        if (!is_close(res[i], expected)) throw std::runtime_error("ReLU GPU Forward failed");
    }
    std::cout << "Passed.\n";
}
...
void test_sgd_gpu() {
    ...
    for(int i=0; i<10; ++i) w_ptr[i] = 1.0f;
    ...
    for(int i=0; i<10; ++i) g_ptr[i] = 0.1f;
    
    Tensor g_gpu = g.clone();
    g_gpu.to_gpu();
    
    // Hack: Copy g_gpu data into w.grad()
    cudaMemcpy(w.grad()->data(), g_gpu.data(), 10*sizeof(float), cudaMemcpyDeviceToDevice);
    ...
    // w = 1.0 - 1.0 * 0.1 = 0.9
    for(int i=0; i<10; ++i) {
        if(!is_close(w_ptr[i], 0.9f)) throw std::runtime_error("SGD GPU update failed");
    }
    std::cout << "Passed.\n";
}
...
void test_training_loop_gpu() {
    ...
    for(int epoch=0; epoch<5; ++epoch) {
        ...
        // We will do a quick manual loss calc/grad gen here or assume mse_loss supports GPU (it needs kernels).
        // For this test, let's pull back to CPU to compute Loss/Grad, then push grad back.
        // In a real optimized engine, loss would be a kernel.
    ...
    if (final_loss >= initial_loss) throw std::runtime_error("GPU Training did not decrease loss");
